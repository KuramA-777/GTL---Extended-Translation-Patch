        ...
        if (event.player.getHeldItem(event.hand) == "kubejs:time_twister_wireless") {
            ...
            if (recipeLogic != null && recipeLogic.isWorking()) {
                ...
                if (eu > 0 && $WirelessEnergyManager.addEUToGlobalEnergyMap(event.player.uuid, $BigInteger.valueOf(- eu))) {
                    ...
                    event.player.setStatusMessage("consumed " + $FormattingUtil.formatNumbers(eu) + " EU，Reduce machine running time " + reducedDuration + " tick")
                }
            }
        }
...
BlockEvents.rightClicked("kubejs:reactor_core", event => {
    if ((event.player.getHeldItem("main_hand") == null && event.player.getHeldItem("off_hand") == null) || event.player.getHeldItem("main_hand") == "minecraft:gold_ingot") {
        ...
        if (dim == "kubejs:ancient_world" || dim == "minecraft:the_nether") {
            ...
            if (event.level.getBlockState(pos.offset(1, 0, 0)).getBlock().getId() == "gtceu:steel_block" && event.level.getBlockState(pos.offset(-1, 0, 0)).getBlock().getId() == "gtceu:steel_block" && event.level.getBlockState(pos.offset(0, 0, 1)).getBlock().getId() == "gtceu:steel_block" && event.level.getBlockState(pos.offset(0, 0, -1)).getBlock().getId() == "gtceu:steel_block" && event.level.getBlockState(pos.offset(1, 0, 1)).getBlock().getId() == "minecraft:diamond_block" && event.level.getBlockState(pos.offset(1, 0, -1)).getBlock().getId() == "minecraft:diamond_block" && event.level.getBlockState(pos.offset(-1, 0, 1)).getBlock().getId() == "minecraft:diamond_block" && event.level.getBlockState(pos.offset(-1, 0, -1)).getBlock().getId() == "minecraft:diamond_block") {
                ...
                event.server.runCommandSilent(`execute in kubejs:ancient_world as ${event.player.getName().getString()} run tp ${0} ${72} ${0}`)
                event.server.runCommandSilent(`execute in kubejs:ancient_world run fill ${0} ${71} ${0} ${0} ${71} ${0} kubejs:reactor_core`)
                ...
                if (event.player.getHeldItem("main_hand") == "minecraft:gold_ingot") {
                    ...
                    event.server.runCommandSilent(`execute as ${event.player.getName().getString()} run tellraw ${event.player.getName().getString()} [{"text":"WARNING: ","color":"red"},{"text":"The Ancient Reactor will always clear a 3x3x5 area below it everytime you use it"}]`)
                    event.server.runCommandSilent(`execute as ${event.player.getName().getString()} run tellraw ${event.player.getName().getString()} [{"text":"DO NOT PLACE ANY BLOCKS HERE, THEY WILL BE CLEARED","color":"red"}]`)
                    event.server.runCommandSilent(`execute in minecraft:the_nether run fill ${-1} ${127} ${1} ${1} ${122} ${-1} minecraft:air`)
                    event.server.runCommandSilent(`execute in minecraft:the_nether run fill ${0} ${127} ${0} ${0} ${127} ${0} kubejs:reactor_core`)
                } else {
                    event.player.setStatusMessage(" Needs a Gold Ingot in your Hand ")
                }
            } else {
